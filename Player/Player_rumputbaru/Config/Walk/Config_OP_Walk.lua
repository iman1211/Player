...
-- Stance parameters
---------------------------------------------
walk.bodyHeight 	= 0.465; --0.295
walk.bodyTilt		= 15*math.pi/180; 
walk.footX		= -0.020; --35 -- 20 
walk.footY 		= 0.035; --0.035
walk.supportX 		= 0;
walk.supportY 		= 0.01;-- -0.05; --0.010;

walk.qLArm=math.pi/180*vector.new({85,20,-40}); --15
walk.qRArm=math.pi/180*vector.new({85,-20,-40});  -- -15

walk.qLArmKick=math.pi/180*vector.new({90,65,-60}); --35 --30
walk.qRArmKick=math.pi/180*vector.new({90,-65,-60}); -- -30

-- walk.hardnessSupport = .80; --.8
-- walk.hardnessSwing 	= .45; --.5
-- walk.hardnessArm 	= .35;--.35; --.3
walk.hardnessSupport = .865; --.8
walk.hardnessSwing 	= .865; --.5 465 --865
walk.hardnessArm 	= .365;--.35; --.3
...
-- Gait parameters
---------------------------------------------
walk.tStep 	= 0.25; --0.28 --0.27 --cepat langkah
walk.tZmp 	= 0.165; --0.155; --0.185 --150
walk.stepHeight = 0.035; --0.06 --0.055 --0.035 --tinggi langkah
...
--------------------------------------------------------------
-- gyroFactor = math.pi/180 * 300 / 1024; --dps to rad/s conversion
gyroFactor = math.pi/180
if Config.servo.pid == 1 then
  walk.ankleImuParamX	= {0.5,0.3*gyroFactor, 1*math.pi/180, 25*math.pi/180};
  walk.ankleImuParamY	= {0.5,0.3*gyroFactor, 1*math.pi/180, 25*math.pi/180};
--   walk.ankleImuParamY	= {0.0,0.0*gyroFactor, 1*math.pi/180, 25*math.pi/180};
...
--  walk.armImuParamY	= {0, 0*gyroFactor, 20*math.pi/180, 45*math.pi/180};
  walk.kneeImuParamX	= {0.5,0.1*gyroFactor, 1*math.pi/180, 25*math.pi/180};
...
walk.supportFront 	= 0.03; --.03; --Lean back when walking fast forward
...
walk.supportBack 	= -0.02; --Lean back when walking backward -0.0015 -0.02  -0.003
walk.supportTurn 	= 0.02; --Lean front when turning 0.02
